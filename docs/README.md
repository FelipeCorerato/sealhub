# 📚 Documentação - SealHub

Índice de toda a documentação técnica do projeto.

## 🚀 Setup e Configuração

- [**FIREBASE_AUTH_SETUP.md**](./FIREBASE_AUTH_SETUP.md) - Configuração da autenticação Firebase
- [**GOOGLE_AUTH_SETUP.md**](./GOOGLE_AUTH_SETUP.md) - Configuração do login com Google
- [**FIRESTORE_RULES_SETUP.md**](./FIRESTORE_RULES_SETUP.md) - Configuração das regras de segurança
- [**EMAIL_DOMAINS_CONFIG.md**](./EMAIL_DOMAINS_CONFIG.md) - Configuração de domínios de email permitidos
- [**EMAIL_VERIFICATION.md**](./EMAIL_VERIFICATION.md) - Sistema de verificação de email
- [**RESUMO_VERIFICACAO_EMAIL.md**](./RESUMO_VERIFICACAO_EMAIL.md) - ⭐ Resumo executivo do sistema de verificação
- [**FIREBASE_EMAIL_CONFIG.md**](./FIREBASE_EMAIL_CONFIG.md) - Configuração e personalização de emails
- [**FIRESTORE_RULES_MULTIPLOS_DOMINIOS.md**](./FIRESTORE_RULES_MULTIPLOS_DOMINIOS.md) - Exemplo de regras para múltiplos domínios
- [**GITHUB_ACTIONS_SETUP.md**](./GITHUB_ACTIONS_SETUP.md) - Configuração de CI/CD

## 📦 Deploy

- [**DEPLOY_FIREBASE.md**](./DEPLOY_FIREBASE.md) - Como fazer deploy no Firebase Hosting
- [**CHECKLIST_DEPLOY.md**](./CHECKLIST_DEPLOY.md) - Checklist pré-deploy
- [**FIX_DEPLOY_ERROR.md**](./FIX_DEPLOY_ERROR.md) - Solução de erros comuns
- [**ERRO_SERVICE_ACCOUNT.md**](./ERRO_SERVICE_ACCOUNT.md) - Erros de service account

## 🗄️ Banco de Dados

- [**FIRESTORE_STRUCTURE.md**](./FIRESTORE_STRUCTURE.md) - Estrutura do Firestore
- [**AUDITORIA_CAMPANHAS.md**](./AUDITORIA_CAMPANHAS.md) - Sistema de auditoria

## 📋 Funcionalidades

- [**CAMPANHAS_SETUP.md**](./CAMPANHAS_SETUP.md) - Como funcionam as campanhas
- [**GERACAO_SELOS.md**](./GERACAO_SELOS.md) - Geração de selos em PDF
- [**COMO_USAR_SELOS.md**](./COMO_USAR_SELOS.md) - Guia de uso dos selos
- [**GUIA_DE_USO.md**](./GUIA_DE_USO.md) - Guia geral do sistema

## 🎨 Interface e UX

- [**Z_INDEX_HIERARCHY.md**](./Z_INDEX_HIERARCHY.md) - Hierarquia de z-index
- [**TESTING_OVERLAYS.md**](./TESTING_OVERLAYS.md) - Testes de overlays
- [**MELHORIAS_ACESSIBILIDADE.md**](./MELHORIAS_ACESSIBILIDADE.md) - ⭐ Melhorias do modal de acessibilidade
- [**OTIMIZACAO_MOBILE.md**](./OTIMIZACAO_MOBILE.md) - ⭐ Otimizações para dispositivos móveis

---

💡 **Dica:** Use Ctrl+F (Cmd+F no Mac) para buscar termos específicos neste índice.

